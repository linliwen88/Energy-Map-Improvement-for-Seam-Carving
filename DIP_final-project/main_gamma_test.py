...
if __name__=='__main__':
    ...
    # Create destiny file
    resultDir = img_name[:-4]+'_gamma_test/'
    ...
    rgbcombine = crop_c(img, 0.8, 'RGBcombine', False)
    ...
    cv2.imwrite(fileName, rgbcombine)
     ...
     # RGB sobel + gamma + color
    print("\nDoing RGB Combine Energy...")
    plt.subplot(233)
    # plt.title("RGB Combine Energy")
    rgbcombine = crop_c(img, 0.8, 'RGBcombine', True)
    plt.imshow(cv2.cvtColor(rgbcombine, cv2.COLOR_BGR2RGB))
    fileName = resultDir + 'rgbcombine_' + str(color_div) + '_gamma.jpg'
    cv2.imwrite(fileName, rgbcombine)


    # LAB sobel + gamma + color
    print("\nDoing LAB Combine Energy...")
    plt.subplot(236)
    # plt.title("LAB Combine Energy")
    labcombine = crop_c(img, 0.8, 'LABcombine', False)
    ...
    plt.subplot(236)
    # plt.title("LAB Combine Energy")
    labcombine = crop_c(img, 0.8, 'LABcombine', True)
    plt.imshow(cv2.cvtColor(labcombine, cv2.COLOR_BGR2RGB))
    fileName = resultDir + 'labcombine_' + str(color_div) + '_gamma.jpg'
    cv2.imwrite(fileName, labcombine)
